...
def format_type_annotation(annotation: type | ForwardRef) -> str:
    """
    ...
    """
    # Handle ForwardRef - return just the string name
    if isinstance(annotation, ForwardRef):
        return annotation.__forward_arg__

    # Handle generic types with ForwardRef arguments
    ...
    if hasattr(annotation, "__module__") and hasattr(annotation, "__name__"):
        ...
        # Types generated by tytr - use just the name
        if module == "tytr.core":
            return name
    ...
    # Remove tytr.core prefix from generated types
    repr_str = repr_str.replace("tytr.core.", "")
...
def generate_recursive_code(
    cls: type,
    name_scheme: dict[str, str] | None = None,
) -> str:
    """
    Generate Python source code for recursive type transformations.

    Parameters
    ----------
    cls : type
        The source class to transform recursively
    name_scheme : dict[str, str], optional
        Name scheme with optional 'prefix' and 'suffix' keys

    Returns
    -------
    str
        Python source code for all generated TypedDicts
    """
    from tytr.core import RenameScheme, to_typeddict_recursive

    # Convert name_scheme dict to RenameScheme if provided
    rename_scheme: RenameScheme | None = None
    if name_scheme is not None:
        rename_scheme = {}
        if "prefix" in name_scheme:
            rename_scheme["prefix"] = name_scheme["prefix"]
        if "suffix" in name_scheme:
            rename_scheme["suffix"] = name_scheme["suffix"]

    # Convert to dict of TypedDicts
    type_dict = to_typeddict_recursive(cls, name_scheme=rename_scheme)

    imports: set[str] = set()
    imports.add("from __future__ import annotations")
    imports.add("from typing import TypedDict")

    # Collect required imports by examining all TypedDicts
    typing_imports = []
    needs_readonly = False

    for td in type_dict.values():
        # Use raw annotations to avoid resolving ForwardRefs
        hints = td.__annotations__
        for field_type in hints.values():
            type_str = format_type_annotation(field_type)
            if "Required[" in type_str and "Required" not in typing_imports:
                typing_imports.append("Required")
    ...
    for td_name, td in type_dict.items():
        hints = td.__annotations__

        body_lines.append(f"class {td_name}(TypedDict):")
        if not hints:
            body_lines.append("    pass")
        else:
            for field_name, field_type in hints.items():
                type_str = format_type_annotation(field_type)
                # Wrap string forward references in quotes for generated code
    ...
    # Assemble final output
    lines: list[str] = []
    lines.append("# Generated by tytr gen recursive")
    lines.extend(sorted(imports))
    lines.append("")
    lines.extend(body_lines)
...
def cmd_gen(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # Generate the code
        if args.transformation == "typeddict" and args.recursive:
            # Build name_scheme dict if prefix/suffix provided
            name_scheme: dict[str, str] | None = None
            if args.prefix or args.suffix:
                name_scheme = {}
                if args.prefix:
                    name_scheme["prefix"] = args.prefix
                if args.suffix:
                    name_scheme["suffix"] = args.suffix
            code = generate_recursive_code(cls, name_scheme=name_scheme)
        else:
            code = generate_code(
...
def main() -> int:
    ...
    gen_parser.add_argument(
        ...
        "-r",
        action="store_true",
        help="Recursively convert nested types (typeddict only)",
    )
    gen_parser.add_argument(
        "--prefix",
        "-p",
        help="Prefix for generated type names (e.g., 'My' -> 'MyClassName')",
    )
    gen_parser.add_argument(
        "--suffix",
        "-s",
        help="Suffix for generated type names (e.g., 'Dict' -> 'ClassNameDict')",
    )
    gen_parser.add_argument(
        "--verbose",
        "-v",
